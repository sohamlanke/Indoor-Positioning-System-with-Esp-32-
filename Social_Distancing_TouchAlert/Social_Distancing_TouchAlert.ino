...
//  digitalWrite(PIN, target_rssi < CUTOFF ? LOW : HIGH);
  if(target_rssi > CUTOFF && detect_flag == 0){
    ...
    if(getLocalTime(&timeinfo)){
      ...
      json.set("/Social_Distancing_Alert"+asString+"/bandid", "84:cc:a8:5e:a5:aa");  //For Band 2
//      json.set("/"+asString+"/bandid", "84:cc:a8:5f:87:5a"); //For Band 1
//      Firebase.updateNode(firebaseData,"/Alert/socialDistancing/"+asString,json);  //This line is depreciated
      }   
    }
    else{
      ...
      if(isTouch() == true){
        json.set("/isTouch","true");
        }
       else{
        json.set("/isTouch", "false");
        }
       Firebase.updateNode(firebaseData,"/Band 2 Alerts/",json);
}
